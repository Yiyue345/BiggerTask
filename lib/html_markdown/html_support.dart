...
void htmlToMarkdown(h.Node? node, int deep, List<m.Node> mNodes) {
  ...
  if (node is h.Text) {
    ...
    if (tag == MarkdownTag.img.name || tag == 'video') {
      ...
      // 清理属性中的单位
      final cleanedAttributes = _cleanAttributes(node.attributes.cast());
      element.attributes.addAll(cleanedAttributes);
    } else {
      element = HtmlElement(tag!, children, node.text);
...
/// 清理属性值中的单位
Map<String, String> _cleanAttributes(Map<String, String> attributes) {
  final cleaned = <String, String>{};
  for (final entry in attributes.entries) {
    if (entry.key == 'width' || entry.key == 'height') {
      // 移除单位（px, em, rem, % 等）
      final cleanedValue = entry.value.replaceAll(RegExp(r'[^0-9.]'), '');
      cleaned[entry.key] = cleanedValue.isEmpty ? '0' : cleanedValue;
    } else {
...
class HtmlToSpanVisitor extends TreeVisitor {
  ...
  void visitElement(h.Element node) {
    ...
    // 清理属性值中的单位
    final cleanedAttributes = _cleanElementAttributes(node.attributes.cast());
    mdElement.attributes.addAll(cleanedAttributes);
  ...
  /// 清理元素属性值中的单位
  Map<String, String> _cleanElementAttributes(Map<String, String> attributes) {
    final cleaned = <String, String>{};
    for (final entry in attributes.entries) {
      if (entry.key == 'width' || entry.key == 'height') {
        // 移除单位（px, em, rem, % 等）
