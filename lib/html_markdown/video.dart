...
class VideoNode extends SpanNode {
  ...
  InlineSpan build() {
    ...
    if (attribute['width'] != null) {
      try {
        // 移除单位（px等）再解析
        String widthStr = attribute['width']!.replaceAll(RegExp(r'[^0-9.]'), '');
        width = double.tryParse(widthStr);
      } catch (e) {
        print("Error parsing width: ${attribute['width']}");
      }
    }
    ...
    if (attribute['height'] != null) {
      try {
        // 移除单位（px等）再解析
        String heightStr = attribute['height']!.replaceAll(RegExp(r'[^0-9.]'), '');
        height = double.tryParse(heightStr);
      } catch (e) {
        print("Error parsing height: ${attribute['height']}");
      }
    }
